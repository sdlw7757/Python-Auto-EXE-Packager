...
class AutoPackGUI:
    def __init__(self, root):
        ...
        self.has_python = self.python_exe is not None

        self.create_ui()
        self.setup_drag_drop()
        
        if not self.has_python:
            messagebox.showwarning("警告", "未检测到Python解释器！\n\n该工具需要Python环境才能运行。")
    
    def find_python_exe(self):
        if getattr(sys, '_MEIPASS', None):
            bundled_py = os.path.join(sys._MEIPASS, "python", "python.exe")
            ...
            for path in [
                ...
                os.path.join(os.environ.get('USERPROFILE', ''), 'AppData', 'Local', 'Programs', 'Python', 'python.exe')
            ]:
    ...
    def create_ui(self):
        ...
        progress_inner = tk.Frame(progress_panel, bg="white")
        progress_inner.pack(padx=15, pady=15, fill=tk.X)
        self.progress = ttk.Progressbar(progress_inner, length=800, mode="determinate")
        self.progress.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.progress_label = tk.Label(progress_inner, text="0%", font=("微软雅黑", 12, "bold"),
                                       bg="white", fg="#2563eb", width=6)
    ...
    def create_venv(self):
        ...
        subprocess.run([self.python_exe, "-m", "venv", self.venv_folder],
...
'''
    ...
    def pack_task(self):
        if not self.has_python:
            messagebox.showerror("错误", "未检测到Python解释器！\n\n该工具需要在安装了Python的电脑上运行。")
            return
        ...
        venv_py = self.python_exe
        using_bundled_python = getattr(sys, '_MEIPASS', None) is not None and self.python_exe != sys.executable
        
        if self.use_venv.get() and not using_bundled_python:
            self.create_venv()
            venv_py = self.get_venv_py()
            self.update_progress(15)
            self.log("正在虚拟环境中安装 PyInstaller ...")
            result = subprocess.run([venv_py, "-m", "pip", "install", "-i",
            ...
            if result.returncode != 0:
                ...
                messagebox.showerror("错误", f"PyInstaller安装失败！\n{result.stderr}")
                return
            self.log("PyInstaller安装成功")
            self.update_progress(20)
        elif using_bundled_python:
            self.log("使用内置Python，跳过虚拟环境创建")
            self.update_progress(15)
        ...
        if using_bundled_python:
            self.log("使用内置Python，优化打包配置...")
            folder = os.path.dirname(py_file)
            sep = ";" if sys.platform == "win32" else ":"
            cmd.extend(["--add-data", f"{folder}{sep}."])
            cmd.extend(["--distpath", os.path.join(folder, "dist")])
            cmd.extend(["--workpath", os.path.join(folder, "build")])
            cmd.extend(["--specpath", folder])
        else:
            folder = os.path.dirname(py_file)
            sep = ";" if sys.platform == "win32" else ":"
            cmd.extend(["--add-data", f"{folder}{sep}."])
